...
for($b=1; $b<= $nbre; $b++)
		...
		$sql31 = 'SELECT COUNT(*) as numb, AppVersion as app FROM data  WHERE  '.$resp2.'  ';
				...
				$sql1 .=' urgency = "Low" AND (type = "Service Request" or type = "Information Request")  AND (AppVersion = "WDI") AND category = "CFF - WDI" AND impacted_account = "Production"   AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
				$sql2 = 'SELECT nom as name from mois where id = "'.$j.'" ';
				
				
				$sql21 .=' urgency = "Medium" AND (type = "Service Request" or type = "Information Request")  AND (AppVersion = "WDI") AND category = "CFF - WDI" AND impacted_account = "Production"   AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
				
				
				$sql31 .=' urgency = "High" AND (type = "Service Request" or type = "Information Request")  AND (AppVersion = "WDI") AND category = "CFF - WDI" AND impacted_account = "Production"   AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
...
$graph->title->SetFont(FF_ARIAL,FS_NORMAL,10);
			$graph->title->Set('CFF WDI  REQUESTS REPARTITION IN PRODUCTION RESOLVED BETWEEN 201'.$annee.'-'.$mois.'-01 AND 201'.$i.'-'.$k  .'-31');
