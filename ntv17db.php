...
$sql5 = 'SELECT tracking_id, short_description, urgency, category, impacted_account, created_date, responsabilite, AppVersion  from data WHERE  '.$resp2.'  urgency = "HIGH" AND created_date >="'.$date3.'" AND created_date <="'.$date2.' 23:59:00" AND (category = "NTV - NSK" OR category = "NTV - YMS") AND impacted_account = "Production" AND type="incident" order by month(created_date)' ;

$sql6 = 'SELECT tracking_id, short_description, urgency, category, impacted_account, created_date, responsabilite, AppVersion  from data WHERE  '.$resp2.'  urgency = "CRITICAL" AND created_date >="'.$date3.'" AND created_date <="'.$date2.' 23:59:00" AND (category = "NTV - NSK" OR category = "NTV - YMS") AND impacted_account = "Production" AND type="incident" order by month(created_date)' ;
...
while ( ($data6 = mysql_fetch_assoc($req6))!== false) {
    echo $data6["AppVersion"]." responsabilité ".$data6["responsabilite"]." (".$data6["tracking_id"]." ) ".$data6["short_description"]."</br> Date: ".$data6["created_date"].".</br>";
}

while ( ($data5 = mysql_fetch_assoc($req5))!== false) {
    echo $data5["AppVersion"]." responsabilité ".$data5["responsabilite"]." (".$data5["tracking_id"]." ) ".$data5["short_description"]."</br> Date: ".$data5["created_date"].".</br>";
}
