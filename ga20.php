...
for($b=1; $b<= $nbre; $b++)
		...
		$sql9 = 'SELECT COUNT(*) as numb, AppVersion as app FROM data  WHERE    ';
				...
				$sql1 .=' urgency = "Low" AND type = "Incident"  AND responsabilite = "NO"  AND (category = "GREATER ANGLIA - YMS"  OR category = "GREATER ANGLIA - YMS") AND impacted_account = "Production" AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
				$sql2 = 'SELECT nom as name from mois where id = "'.$j.'" ';
				$sql3 .=' urgency = "Low" AND type = "Incident"  AND responsabilite = "YES"  AND (category = "GREATER ANGLIA - YMS"  OR category = "GREATER ANGLIA - YMS") AND impacted_account = "Production" AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
				$sql4 .=' urgency = "Medium" AND type = "Incident"  AND responsabilite = "NO" AND (category = "GREATER ANGLIA - YMS"  OR category = "GREATER ANGLIA - YMS") AND impacted_account = "Production" AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
				$sql5 .=' urgency = "Medium" AND type = "Incident"  AND responsabilite = "YES" AND (category = "GREATER ANGLIA - YMS"  OR category = "GREATER ANGLIA - YMS") AND impacted_account = "Production" AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
				$sql6 .=' urgency = "High" AND type = "Incident"  AND responsabilite = "NO" AND (category = "GREATER ANGLIA - YMS"  OR category = "GREATER ANGLIA - YMS") AND impacted_account = "Production" AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
				$sql7 .=' urgency = "High" AND type = "Incident"  AND responsabilite = "YES" AND (category = "GREATER ANGLIA - YMS"  OR category = "GREATER ANGLIA - YMS") AND impacted_account = "Production" AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31 23:59:00"' ;
...
$graph->title->SetFont(FF_ARIAL,FS_NORMAL,10);
			$graph->title->Set('GREATER ANGLIA INCIDENTS RESPONSABILITY IN PRODUCTION BETWEEN 201'.$annee.'-'.$mois.'-01 AND 201'.$i.'-'.$k  .'-31');
...
$b1plot->SetLegend('GREATER ANGLIA Side');
