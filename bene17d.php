...
for($b=1; $b<= $nbre; $b++)
		...
		$sql31 = 'SELECT COUNT(*) as numb, AppVersion as app FROM data  WHERE  '.$resp2.'  ';
				...
				$sql1 .=' urgency = "Low" AND type = "Incident"   AND (category = "BENERAIL - WDI" OR category = "BENERAIL - WDI") AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				$sql2 = 'SELECT nom as name from mois where id = "'.$j.'" ';
				
				
				$sql21 .=' urgency = "Medium" AND type = "Incident"   AND (category = "BENERAIL - WDI" OR category = "BENERAIL - WDI") AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				
				
				$sql31 .=' urgency = "High" AND type = "Incident"  AND (category = "BENERAIL - WDI" OR category = "BENERAIL - WDI") AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
