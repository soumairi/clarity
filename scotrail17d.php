...
for($b=1; $b<= $nbre; $b++)
		...
		$sql31 = 'SELECT COUNT(*) as numb, AppVersion as app FROM data  WHERE  '.$resp2.'  ';
				...
				$sql1 .=' urgency = "Low" AND type = "Incident"  AND category = "SCOTRAIL - YMS" AND impacted_account = "Production"   AND created_date >="201'.$i.'-'.$j.'-01" AND created_date <="201'.$i.'-'.$j.'-31 23:59:00"' ;
				$sql2 = 'SELECT nom as name from mois where id = "'.$j.'" ';
				
				
				$sql21 .=' urgency = "Medium" AND type = "Incident"  AND category = "SCOTRAIL - YMS" AND impacted_account = "Production"   AND created_date >="201'.$i.'-'.$j.'-01" AND created_date <="201'.$i.'-'.$j.'-31 23:59:00"' ;
				
				
				$sql31 .=' urgency = "High" AND type = "Incident"  AND category = "SCOTRAIL - YMS" AND impacted_account = "Production"   AND created_date >="201'.$i.'-'.$j.'-01" AND created_date <="201'.$i.'-'.$j.'-31 23:59:00"' ;
...
$graph->title->SetFont(FF_ARIAL,FS_NORMAL,10);
			$graph->title->Set('SCOTRAIL  INCIDENTS REPARTITION IN PRODUCTION CREATED BETWEEN 201'.$annee.'-'.$mois.'-01 AND 201'.$i.'-'.$k  .'-31');
