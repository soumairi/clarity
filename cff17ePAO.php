...
for($b=1; $b<= $nbre; $b++)
		...
		$sql31 = 'SELECT COUNT(*) as numb, AppVersion as app FROM data  WHERE  '.$resp2.'  ';
				...
				$sql1 .=' urgency = "Low" AND type = "Incident"   AND (category = "CFF - PAO" OR category = "CFF - PAO") AND impacted_account = "Production"   AND created_date <"20'.$i.'-0'.$j.'-31 23:59:00" AND (resolution_date >"20'.$i.'-0'.$j.'-31 23:59:00" OR resolution_date ="0000-00-00 00:00:00 ") AND status <> "Rejected"';
				$sql2 = 'SELECT nom as name from mois where id = "'.$j.'" ';
				
				
				$sql21 .=' urgency = "Medium" AND type = "Incident"   AND (category = "CFF - PAO" OR category = "CFF - PAO") AND impacted_account = "Production"   AND created_date <"20'.$i.'-0'.$j.'-31 23:59:00" AND (resolution_date >"20'.$i.'-0'.$j.'-31 23:59:00" OR resolution_date ="0000-00-00 00:00:00 ") AND status <> "Rejected"';
				
				
				$sql31 .=' urgency = "High" AND type = "Incident"  AND (category = "CFF - PAO" OR category = "CFF - PAO") AND impacted_account = "Production"   AND created_date <"20'.$i.'-0'.$j.'-31 23:59:00" AND (resolution_date >"20'.$i.'-0'.$j.'-31 23:59:00" OR resolution_date ="0000-00-00 00:00:00 ") AND status <> "Rejected"';
...
$graph->title->SetFont(FF_ARIAL,FS_NORMAL,10);
			$graph->title->Set('CFF INCIDENTS REPARTITION IN PRODUCTION STILL OPENED BETWEEN 20'.$annee.'-'.$mois.'-01 AND 20'.$i.'-'.$k  .'-31');
