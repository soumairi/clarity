...
for($b=1; $b<= $nbre; $b++)
				...
				$sql4 = 'SELECT  avg(datediff( resolution_date, created_date) - (WEEK(resolution_date) - WEEK(created_date))* 2 - IFNULL((datediff(end_bounced_time, start_bounced_time) - (WEEK(end_bounced_time) - WEEK(start_bounced_time))*2),0))  as numb from data WHERE  '.$resp2.'  ';
								...
								//for ($j=$mois; $j <= $mois2 ; $j++ )
									...
									$sql1 .=' urgency = "low" AND resolution_date >="20'.$i.'-0'.$j.'-01" AND resolution_date <="20'.$i.'-0'.$j.'-31" AND (category = "BENERAIL - PAO" OR category = "BENERAIL - PAO") AND impacted_account = "Production" AND type="incident" ' ;
									$sql2 = 'SELECT nom as name from mois where id = "'.$j.'"';
									$sql3 .=' urgency = "medium" AND resolution_date >="20'.$i.'-0'.$j.'-01" AND resolution_date <="20'.$i.'-0'.$j.'-31" AND (category = "BENERAIL - PAO" OR category = "BENERAIL - PAO") AND impacted_account = "Production" AND type="incident"' ;
									$sql4 .=' urgency = "high" AND resolution_date >="20'.$i.'-0'.$j.'-01" AND resolution_date <="20'.$i.'-0'.$j.'-31" AND (category = "BENERAIL - PAO" OR category = "BENERAIL - PAO") AND impacted_account = "Production" AND type="incident"' ;
