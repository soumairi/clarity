...
for($b=1; $b<= $nbre; $b++)
		...
		$sql31 = 'SELECT COUNT(*) as numb, AppVersion as app FROM data  WHERE  '.$resp2.'  ';
				...
				$sql1 .=' urgency = "Low" AND type = "Incident"   AND (category = "BENERAIL - PAO" or category = "BENERAIL - PAO") AND impacted_account = "Production"   AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="20'.$i.'-0'.$j.'-01" AND resolution_date <="20'.$i.'-0'.$j.'-31 23:59:00"' ;
				$sql2 = 'SELECT nom as name from mois where id = "'.$j.'" ';
				
				
				$sql21 .=' urgency = "Medium" AND type = "Incident"   AND (category = "BENERAIL - PAO" or category = "BENERAIL - PAO") AND impacted_account = "Production"   AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="20'.$i.'-0'.$j.'-01" AND resolution_date <="20'.$i.'-0'.$j.'-31 23:59:00"' ;
				
				
				$sql31 .=' urgency = "High" AND type = "Incident"   AND (category = "BENERAIL - PAO" or category = "BENERAIL - PAO") AND impacted_account = "Production"   AND (status = "Resolved" OR status = "Temporary Resolved" OR status = "Closed" ) AND resolution_date >="20'.$i.'-0'.$j.'-01" AND resolution_date <="20'.$i.'-0'.$j.'-31 23:59:00"' ;
