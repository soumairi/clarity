...
for($b=1; $b<= $nbre; $b++)
				...
				$sql4 = 'SELECT  avg(datediff( resolution_date, created_date) - (WEEK(resolution_date) - WEEK(created_date))* 2 - IFNULL((datediff(end_bounced_time, start_bounced_time) - (WEEK(end_bounced_time) - WEEK(start_bounced_time))*2),0))  as numb from data WHERE  '.$resp2.'  ';
								...
								//for ($j=$mois; $j <= $mois2 ; $j++ )
									...
									$sql1 .=' urgency = "low" AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31" AND (category = "NTV - NSK" OR category = "NTV - YMS") AND impacted_account = "Production" AND (type = "Service Request" OR type = "Information Request")    ' ;
									$sql2 = 'SELECT nom as name from mois where id = "'.$j.'"';
									$sql3 .=' urgency = "medium" AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31" AND (category = "NTV - NSK" OR category = "NTV - YMS") AND impacted_account = "Production" AND (type = "Service Request" OR type = "Information Request")   ' ;
									$sql4 .=' urgency = "high" AND resolution_date >="201'.$i.'-0'.$j.'-01" AND resolution_date <="201'.$i.'-0'.$j.'-31" AND (category = "NTV - NSK" OR category = "NTV - YMS") AND impacted_account = "Production" AND (type = "Service Request" OR type = "Information Request")   ' ;
...
// content="text/plain; charset=utf-8" 
			...
			$graph->title->Set($valeur." REQUESTS AVERAGE LIFE TIME EVOLUTION IN PRODUCTION");
