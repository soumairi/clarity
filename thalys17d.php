...
for($b=1; $b<= $nbre; $b++)
		...
		$sql2 = 'SELECT COUNT(*) as numb, AppVersion as app FROM data  WHERE  '.$resp2.'  ';
		...
		$sql10 = 'SELECT COUNT(*) as numb, AppVersion as app FROM data  WHERE  '.$resp2.'  ';
				...
				$sql1 .=' urgency = "Low" AND type = "Incident"  AND AppVersion = "COUCHE MEDIATION"  AND category = "THALYS - YMS"  AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				$sql2 = 'SELECT nom as name from mois where id = "'.$j.'" ';
				$sql3 .=' urgency = "Low" AND type = "Incident"  AND (AppVersion= "APPIA GUI" OR AppVersion= "APPIA DATABASE"  OR AppVersion= "APPIA OPTIMIZATION" OR AppVersion="AU UPDATE FILE" OR AppVersion="RFFE FILE" OR AppVersion="NETWORK" OR AppVersion="APPIA IHM" OR AppVersion= "RMF SVSI") AND category = "THALYS - YMS" AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				$sql4 .=' urgency = "Low" AND type = "Incident"  AND (AppVersion = "COUCHE MEDIATION" OR AppVersion= "RESA RAIL" OR AppVersion= "RR-INTERMED"  OR AppVersion= "BCTH" OR AppVersion="NETWORK" OR AppVersion="OCRE") AND category = "THALYS-RR" AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				
				$sql5 .=' urgency = "Medium" AND type = "Incident"  AND AppVersion = "COUCHE MEDIATION"  AND category = "THALYS - YMS"  AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				$sql6 .=' urgency = "Medium" AND type = "Incident"  AND (AppVersion= "APPIA GUI" OR AppVersion= "APPIA DATABASE"  OR AppVersion= "APPIA OPTIMIZATION" OR AppVersion="AU UPDATE FILE" OR AppVersion="RFFE FILE" OR AppVersion="NETWORK" OR AppVersion="APPIA IHM" OR AppVersion= "RMF SVSI") AND category = "THALYS - YMS"  AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				$sql7 .=' urgency = "Medium" AND type = "Incident"  AND (AppVersion = "COUCHE MEDIATION" OR AppVersion= "RESA RAIL" OR AppVersion= "RR-INTERMED"  OR AppVersion= "BCTH" OR AppVersion="NETWORK" OR AppVersion="OCRE") AND category = "THALYS-RR" AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				
				$sql8 .=' urgency = "High" AND type = "Incident"  AND AppVersion = "COUCHE MEDIATION"  AND category = "THALYS - YMS"  AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				$sql9 .=' urgency = "High" AND type = "Incident"  AND (AppVersion= "APPIA GUI" OR AppVersion= "APPIA DATABASE"  OR AppVersion= "APPIA OPTIMIZATION" OR AppVersion="AU UPDATE FILE" OR AppVersion="RFFE FILE" OR AppVersion="NETWORK" OR AppVersion="APPIA IHM" OR AppVersion= "RMF SVSI") AND category = "THALYS - YMS"  AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
				$sql10 .=' urgency = "High" AND type = "Incident"  AND (AppVersion = "COUCHE MEDIATION" OR AppVersion= "RESA RAIL" OR AppVersion= "RR-INTERMED"  OR AppVersion= "BCTH" OR AppVersion="NETWORK" OR AppVersion="OCRE" OR AppVersion= "RMF SVSI") AND category = "THALYS-RR" AND impacted_account = "Production"   AND created_date >="20'.$i.'-'.$j.'-01" AND created_date <="20'.$i.'-'.$j.'-31 23:59:00"' ;
...
$graph->title->SetFont(FF_ARIAL,FS_NORMAL,10);
			$graph->title->Set('THALYS - YMS - RR APP REPARTITION IN PRODUCTION INCIDENTS CREATED SINCE 20'.$annee.'-'.$mois.'-01 AND 20'.$i.'-'.$k  .'-31');
